...
##################################################################### 
#### Up to here, whole code above was provided by my supervisor #####
##################################################################### 
...
def solve(model, time_limit, mip_gap, tee_value):
    ...
    if pyomo_result.solver.status == SolverStatus.ok and pyomo_result.solver.termination_condition == TerminationCondition.optimal:
        ...
        # Check if solver was aborted due to time limit but found a solution
        if pyomo_result.solver.termination_condition == TerminationCondition.maxTimeLimit:

            print("Solver was aborted due to time limit but found a solution.")
